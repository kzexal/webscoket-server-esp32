...
export interface GeminiVoiceAgentOptions {
    apiKey?: string;
    instructions?: string;
...
export class GeminiVoiceAgent {
    ...
    constructor(params: GeminiVoiceAgentOptions) {
    ...
    public async connect(
        ...
        console.log("GeminiVoiceAgent connected");
            ...
            try {
                ...
                if (Buffer.isBuffer(data)) {
                    if (this.isRecording) {
                        ...
                        // #region agent log
                        fetch('http://127.0.0.1:7243/ingest/04558b8f-606a-46fc-b607-0ccd441ef8fa', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sessionId: 'debug-session', runId: 'post-fix', hypothesisId: 'H4', location: 'src/lib/gemini_agent.ts:121', message: 'binary message ignored because not recording', data: { size: data.length }, timestamp: Date.now() }) }).catch(() => { });
                        // #endregion
                    ...
                    if (this.isRecording) {
                        ...
                        // #region agent log
                        fetch('http://127.0.0.1:7243/ingest/04558b8f-606a-46fc-b607-0ccd441ef8fa', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sessionId: 'debug-session', runId: 'post-fix', hypothesisId: 'H4', location: 'src/lib/gemini_agent.ts:126', message: 'binary message ignored because not recording', data: { size: buffer.length }, timestamp: Date.now() }) }).catch(() => { });
                        // #endregion
                        this.debugLoggedNonRecording = true;
                    }
    ...
    private async handleControlMessage(
        ...
        // #region agent log
        fetch('http://127.0.0.1:7243/ingest/04558b8f-606a-46fc-b607-0ccd441ef8fa', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sessionId: 'debug-session', runId: 'initial', hypothesisId: 'H3', location: 'src/lib/gemini_agent.ts:150', message: 'handleControlMessage invoked', data: { type: message?.type }, timestamp: Date.now() }) }).catch(() => { });
        // #endregion
    ...
    //Gửi đến Gemini, nhận phản hồi và gửi lại
